...
function calculateStats(input) {
    const { gameId, attendanceHistory, benefitHistory, activePopulationHistory, arrivalsHistory, departuresHistory, capacity, numAgents, benefitRules, } = input;
    const totalRounds = attendanceHistory.length;
    const resolvedActivePopulationHistory = activePopulationHistory && activePopulationHistory.length === totalRounds
        ? activePopulationHistory
        : Array.from({ length: totalRounds }, () => numAgents);
    const resolvedArrivalsHistory = arrivalsHistory && arrivalsHistory.length === totalRounds
        ? arrivalsHistory
        : Array.from({ length: totalRounds }, () => 0);
    const resolvedDeparturesHistory = departuresHistory && departuresHistory.length === totalRounds
        ? departuresHistory
        : Array.from({ length: totalRounds }, () => 0);
    if (totalRounds === 0) {
        return {
    ...
    const averageActivePopulation = resolvedActivePopulationHistory.reduce((sum, val) => sum + val, 0) / totalRounds;
    ...
    const minBenefit = -resolvedActivePopulationHistory.reduce((sum, activeCount) => sum + activeCount, 0) * negativeMultiplier;
    ...
    const averageParticipationRate = activeAgentRounds > 0
