...
Альтернативный пример: студент пытается выбрать КПВ в smartway, но сервер упал. Студент может каждую минуту либо повторить попытку и потерять время, либо не делать этого и получить полезность от чего-либо. Каждую минуту он решает пойти или нет - если идет большее число студентов чем вместимость сервера, то он продолжает падать.
